...
const [programmerId, setProgrammerId] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    return programmer[programmerId -1 ].name
  }; // we do a minus 1 because if not it will dispolay the name of the developer that comes after the one you chose BECAUSE the array is 0 indexed so it will always be behind if you dont do -1
console.log([programmer])
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
   color: programmerId !== null ? 'gold' : 'royalblue'
  }; // saying that if the programmerId is not null make the color gold and if it is null make ir royalblue
      ...
      <div className='programmers'>
        {
          ...
          ) // mapped over programmer instead 
        }
      </div>
