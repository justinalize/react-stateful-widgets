...
const [count, setCount] = useState(0) // creating a use state for a counter that initializes at 0


  const increment = () => {
    setCount(count + 1) // saying that if i press the button the count will add one to itself
  };
  const decrement = () => {
   setCount(count - 1) // saying that if i press the button the count will subtract one from itself
  };
  const reset = () => {
    setCount(0) // saying that if i press reset the counter will return to 0
  };
  ...
  }; // saying that if count is even to change the color to royalblue and if its odd to make it crimson
